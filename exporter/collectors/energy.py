...
#
# Power estimation: GPU (TDP * util%) + CPU (TDP * util%)
# Energy formula:  E (kWh) = P (W) * dt (s) / 3600 / 1000
# Cost formula:    C (EUR) = E (kWh) * price (EUR/kWh)
#
# Italian energy tariff bands (configurabile in .env o costanti):
...
# Source: ARERA standard reference prices, configurable
TARIFF = {
    'F1': 0.28,   # peak
    'F2': 0.22,   # mid
    'F3': 0.16,   # off-peak
...
def collect(session: str, gpu_power_w: float, cpu_power_w: float):
    ...
    total_w   = gpu_power_w + cpu_power_w
